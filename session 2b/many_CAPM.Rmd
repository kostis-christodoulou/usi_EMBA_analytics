---
title: "CAPM for DJIA and SPY components"
author: "CA09 - Data Analytics for Finance"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
# Scrape wikipedia to get stock market symbols (tickers) 
...
# Download adjusted closing prices for all DJIA stocks
...
# `purr::map()` to run 30 regressions to estimate betas of all 30 DJIA stocks 

```{r djia_CAPM, warning=FALSE, cache=TRUE}
...
ggplot(data = slopes, aes(x = symbol, y = estimate))+
  ...
  labs(title = 'Estimating betas of stocks in the DJIA',
       ...
       x = "",
       y = "beta estimate") +
  geom_hline(yintercept = 1, colour= "#001e62")+
...
# Scrape Wikipedia to get all ticker symbols for the SP500
...
# Download stock price data for all SP500 stocks
...
```{r sp500_CAPM, warning=FALSE, message=FALSE, cache=TRUE}
#calculate monthly  returns
...
# Where are the SP500 companies headquartered
capm_results %>% 
   count(headquarters_location, sort=TRUE) %>% 
  View()
...
# save file with stock data, that you could read using vroom::vroom()
# write_csv(sp500_stocks, "data/sp500_stocks.csv")
```
