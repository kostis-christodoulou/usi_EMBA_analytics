...
4.  Using the commands below, create a new dataframe called `us_atheism` that contains
    only the rows in `atheism` associated with US respondents to the 2005 and 2012 survey 
...
the standard error and construct the interval by hand, or allow mosaic's `prop.test(k,n)`
and `confint` functions to do it for us.
...
7.  Using the same approach, calculate confidence intervals for the 
...
10. Repeat the above simulation three more times but with modified sample
    ...
    and describe the three new sampling distributions. 
...
-   Answer the following two questions using the `prop.test` and `confint` functions. As 
