...
return_on_budget_formula_ci <- movies %>%
  filter(genre %in% genres_to_choose) %>% 
  group_by(genre) %>% 
  summarise(mean_return = mean(return_on_budget),
            median_return = median(return_on_budget),
            sd_return = sd(return_on_budget),
            count = n(),
            # get t-critical value with (n-1) degrees of freedom
            t_critical = qt(0.975, count-1),
            se_return = sd_return/sqrt(count),
            margin_of_error = t_critical * se_return,
            rating_low = mean_return - margin_of_error,
            rating_high = mean_return + margin_of_error
  ) %>% 
  arrange(desc(median_return))


return_on_budget_formula_ci
#**************************************
